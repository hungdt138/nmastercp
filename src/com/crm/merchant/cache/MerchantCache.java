...
public class MerchantCache {
	...
	protected void loadMerchant(Connection connection) throws Exception {
		...
		try {
			...
			while(rsConfig.next()){
				log.debug(String.format("Caching merchant rule for %s ...", rsConfig.getString("alias_")));
				MerchantEntry merchant = new MerchantEntry(rsConfig.getLong("merchantId"), Database.getString(rsConfig, "alias_"));
				merchant.setMerchantId(rsConfig.getLong("merchantId"));
				merchant.setAlias(rsConfig.getString("alias_"));
				...
				log.debug(String.format("Merchant rule %s is cached", rsConfig.getString("alias_")));
