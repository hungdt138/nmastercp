...
public class VinaphoneGetCDRThread extends DispatcherThread {
	...
	public Vector getParameterDefinition() {
		...
		vtReturn.add(ThreadUtil
				.createTextParameter("ftpUse", 400,
						"The billing runing ftp mode, that must put cdr file in remote server"));
	...
	public void fillDispatcherParameter() throws Exception {
		...
		ftpClientFolder = ThreadUtil.getString(this, "ftpClientFolder", true,
				"");
		ftpDownloadInterval = ThreadUtil.getInt(this, "ftpDownloadInterval",
				3000);
		ftpPreFile = ThreadUtil.getString(this, "ftpPreFile", true, "");
		ftpTimetorun = ThreadUtil.getString(this, "ftpTimetorun", false, "");
		startTimeInDay = ThreadUtil.getInt(this, "startTimeInDay", 5);
		endTimeInDay = ThreadUtil.getInt(this, "endTimeInDay", 7);
				...
				for (FTPFile file : files) {
					...
					if (!filePathLocal.exists()) {
						if (file.getName().startsWith(ftpPreFile)) {
							...
							if (!ftp.retrieveFile(remoteFile + "/"
									+ remoteFile1, outputStream1)) {
								logMonitor("Unable to download file from FTP server : "
										+ file.getName());
								File tmp = null;
								try {
									outputStream1.close();
									...
									logMonitor("Unable to delete corrupt file from local directory : "
											+ tmp.getAbsolutePath());
								}
							} else {
								...
								outputStream1.close();
								count++;
								logMonitor("Download file " + file.getName()
										+ " success from " + remoteFile
										+ " to " + localFile);

							}
						}
					} else {
						logMonitor("File " + file.getName() + " exist in "
								+ localFile);
					}
