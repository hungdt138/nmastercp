...
public class CommandUtil
{
	...
	public static String calculateCostTime(Date startTime, Date endTime)
	{
		long timeCost = (endTime != null ? endTime.getTime() : 0) - (startTime != null ? startTime.getTime() : 0);
		return "" + timeCost;
		// if (timeCost < Constants.COST_LOW_VALUE)
		// return Constants.COST_LOW;
		// else if (timeCost > Constants.COST_HIGH_VALUE)
		// return Constants.COST_HIGH;
		// else
		// return Constants.COST_MEDIUM;
	}
	...
	{
		if (request.getParameters().getBoolean("isViettel_nonsub"))
		{
			sendSMSVT(instance, request, request.getServiceAddress(), request.getShipTo(), content);
		}
		else
		{
			sendSMS(instance, request, request.getServiceAddress(), request.getShipTo(), content);
		}

	}

	public static CommandMessage sendSMSVT(DispatcherInstance instance,
			CommandMessage request, String serviceAddress, String shippingTo,
			String content) throws Exception
	{
		...
		CommandMessage requestMO = new CommandMessage();

		try
		{

			// provisioning & command
			CommandEntry command = ProvisioningFactory.getCache().getCommand("VT8X26.SENDSMS");

			requestMO.setProvisioningType("VT8X26");
			requestMO.setCommandId(command.getCommandId());
			requestMO.setIsdn(request.getIsdn());
			requestMO.setOrderId(request.getOrderId());
			requestMO.setRequestId(request.getRequestId());
			requestMO.getParameters().setString("vt8x26.commandcode", request.getParameters().getString("vt8x26.commandcode", "GET"));
			...
			requestMO.setNextQuestion(content);

			requestMO.setRequestValue(ResponseUtil.SMS_CMD_CHECK, "false");

			QueueFactory.attachCommandRouting(requestMO);
			// QueueFactory.attachLocal("queue/VT8X26", requestMO);
