...
public class QueueFactory
{
	...
	public static String									VT_ORDER_RESPONSE_QUEUE		= "vt/OrderResponse";
	public static String									SUB_ORDER_RESPONSE_QUEUE	= "sub/OrderResponse";
	...
	public static String									DAILY_RENEW					= "queue/DailyRenew";

	public static String									ALARM_QUEUE					= "queue/Alarm";

	public static String									MONITOR_QUEUE				= "queue/MonitorLog";
	...
	public static LocalQueue getLocalQueue(String queueName)
	{
		...
		if (queue == null)
		{
			...
			if (queueName.equals(COMMAND_LOG_QUEUE) || queueName.equals(MONITOR_QUEUE)
					|| queueName.equals(COMMAND_STATISTIC_QUEUE) || queueName.equals(ORDER_RESPONSE_QUEUE))
			{
				checkPending = false;
			}
	...
	public static void attachMonitor(MonitorMessage request) throws Exception
	{
		if (getLocalQueueSize(MONITOR_QUEUE) > 50000)
		{
			detachLocal(MONITOR_QUEUE);
		}
		attachLocal(MONITOR_QUEUE, request);
	}
	...
	public static synchronized void initContext() throws Exception
	{
		...
		synchronized (mutex)
		{
			getLocalQueue(MONITOR_QUEUE).setCheckPending(false);
			...
			try
			{
				if (context == null)
				{
					...
					DAILY_RENEW = configProvider.getString("queue.dailyRenew", "queue/DailyRenew");
	...
	public static void initLocalQueue() throws Exception
	{
		...
		try
		{
			...
			ORDER_REQUEST_QUEUE = configProvider.getString("queue.orderRoute", "queue/OrderRoute");
			ORDER_RESPONSE_QUEUE = configProvider.getString("queue.orderResponse", "queue/OrderResponse");
			VT_ORDER_RESPONSE_QUEUE = configProvider.getString("vt.orderResponse", "vt/OrderResponse");
			SUB_ORDER_RESPONSE_QUEUE = configProvider.getString("sub.orderResponse", "sub/OrderResponse");
			COMMAND_STATISTIC_QUEUE = configProvider.getString("queue.commandStatistic", "queue/CommandStatistic");
			ALARM_QUEUE = configProvider.getString("queue.alarm", "queue/Alarm");
