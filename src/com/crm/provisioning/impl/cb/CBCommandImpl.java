...
public class CBCommandImpl extends CommandImpl
{
	...
	//DuyMB add check subscriber status for postpaid
	public VNMMessage checkSubscriberStatus(CommandInstance instance,
			...
			throws Exception
	{
		...
		double  commandResult = -1;
		try
		{
			String isdn = transformISDN(request.getIsdn());
			connection = (CBConnection) instance.getProvisioningConnection(); 
			String schema = connection.getSchema(getSchema(request));
			commandResult = connection.getSubscriberStatus(schema, isdn, request.getCont_code());
			instance.getDispatcher().logMonitor("isdn: " + isdn + " Status CB:" + commandResult + " ContCode:" + request.getCont_code());
			if (commandResult == 1)
			{
				result.setCause(Constants.SUCCESS);
				result.setCauseValue((int)commandResult);
				result.setStatus(Constants.ORDER_STATUS_APPROVED);
			}
			else
			{
				if (request.getCont_code() == 102)
				{
					result.setCause(Constants.BALANCE_STATE_SUSPEND_S1);
					result.setCauseValue((int)commandResult);
					result.setStatus(Constants.ORDER_STATUS_DENIED);					
				}else if (request.getCont_code() == 103)
				{
					result.setCause(Constants.BALANCE_STATE_SUSPEND_S2);
					result.setCauseValue((int)commandResult);
					result.setStatus(Constants.ORDER_STATUS_DENIED);					
				}
			}
		...
		catch (Exception e)
		{
			...
			result.setCauseValue((int)commandResult);	
			result.setStatus(Constants.ORDER_STATUS_DENIED);
			throw new Exception("Check status error: " + e.getMessage());
		}
		finally
		{
		...
		else
		{
			try
			{
				...
				try
				{
					...
					connection.checkBalancePostpaid(transformISDN(request.getIsdn()),
							balances);
					request.setResponse(getLogResponse(balances));

					instance.debugMonitor("RECEIVE:" + request.getResponse() + ". ID="
							+ sessionId);
				}
				finally
				{
				...
				else if (request.getChannel().equals(Constants.CHANNEL_WEB))
				{
					result.setResponseValue(ResponseUtil.AMOUNT_OUTSTANDING,
							balances[0]);
					result.setResponseValue(ResponseUtil.AMOUNT_UNBILL, balances[1]);
		...
		else
		{
			try
			{
				...
				try
				{
					connection = (CBConnection) instance.getProvisioningConnection();
					String schema = connection.getSchema(getSchema(request));
					cbStatus = connection.getServiceStatus(schema, serviceTypeId, productId, isdn, respProperties);
					...
					if (cbStatus == 2 | cbStatus == 3)
					{
						...
						if (!request.isPostpaid() && createPrepaidInfor(request, "createSubscriber"))
						{
							Long baseProductInstanceId =
									connection.getBaseProductInstanceId(schema, isdn, serviceTypeId);

							if (baseProductInstanceId < 0)
							{
								SubscriberEntryImpl.getSubscriberInfo(request);
								connection.createPrepaidSubscriber(isdn, request, respProperties);
							}
						}
						...
						String strCommand = "REGISTER(" + isdn + "), CB_STATUS=" + cbStatus + ", PRODUCTID=" + productId + ";";
						long sessionId = setRequest(instance, request, strCommand);
			...
			try
			{
				...
				try
				{
					connection = (CBConnection) instance
							.getProvisioningConnection();
					String schema = connection.getSchema(getSchema(request));
					cbStatus = connection.getServiceStatus(schema, serviceTypeId,
							productId, isdn, respProperties);
					...
					else if (cbStatus == 1)
					{
						if (subscriberProduct != null)
						{
							String strCancelMode = getCancelMode(request);

							String strCommand = "UNREGISTER(" + isdn + "), CB_STATUS=" + cbStatus + ", PRODUCTID=" + productId;
							...
							long sessionId = setRequest(instance, request, strCommand);
							if (strCancelMode.equals(""))
								responseCode = connection.unregisterService(isdn, productId);
							else
							{
			...
			throws Exception
	{
		...
		else
		{
			try
			{
				...
				double timeUsed = connection.getTimeUsed(schema, result.getIsdn(),
						sdf.format(subProduct.getRegisterDate()));
