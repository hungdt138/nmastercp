...
public class ChangeBalance extends Charging
{
	...
	public void setContent(String strContent) throws Exception
	{
		...
		try
		{
			...
			{
				if (!content[i].split("=")[0].substring(8).equals(content[i + 1].split("=")[0].substring(11)))
				{
				...
				if (_tempAmout.equals(""))
				{
					_tempAmout = "0";

				}
				if (_tempExpiredate.equals(""))
				{
					_tempExpiredate = "0";
				}
	...
	public String toString()
	{
		String returnStr = getCharg_seq() + "," + getId() + "," + getAccount() + "," + getMdn() + "," +
				getBalance() + "," + getAmount() + "," + getExpireDate() + "," + getComment();
		return returnStr;
	}
