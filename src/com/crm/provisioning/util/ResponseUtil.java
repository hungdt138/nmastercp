...
public class ResponseUtil
{
			...
			DispatcherInstance instance, ProductEntry product, CommandMessage request, String actionType, String template)
	{
		...
		try
		{
			...
			content = content.replaceAll("~NUMOFCODE~", request.getParameters().getString("NumOfCode"));
			// Dinh kem cau hoi
			content = content.replaceAll("~QUESTION~", request.getNextQuestion());
			...
			, String actionType, String cause, String prefix, String postfix, String isdn)
	{
		...
		try
		{
			...
			if (!content.equals(""))
			{
				...
				if (product != null && product.getParameter("sendfirstcontent", "false").contains("true"))
				{
					content = request.getParameters().getString("firstcontent");
					if (!content.equals(""))
					{
						Thread.sleep(1000);
						CommandUtil.sendSMS(instance, request, content);
				 ...
				 */
				if (product != null && product.getParameter(request.getActionType() + ".feedbackanswer", "false").equals("true")
						&& (request.getActionType().equals(Constants.ACTION_REGISTER) || request.getActionType().equals(Constants.ERROR_REGISTERED)))
				{
