...
public class BQSCommandImpl extends CCWSCommandImpl
{
			...
			throws Exception
	{
		...
		try
		{
			...
			boolean getCOSOnly = config.getBoolean("COSOnly", false);

			if (getCOSOnly)
			{
				content = config.getString("COSOnly.message", "Goi cuoc cua quy khach la <COS_NAME>.");

				content = content.replace("<COS_NAME>", subscriber.getCOSName());
			}
			else
			{
				header = config.getString("header", "");
				tailer = config.getString("tailer", "");
				...
				// get SMS content
				// String defaultUnit = getProperty(config, "", "", "unit",
				// "d");
			...
			throws Exception
	{
		...
		try
		{
			...
			subscriber = ((VNMMessage)request).getSubscriberRetrieve();

			long endTime = System.currentTimeMillis();

			String strResponse = "";
			if (subscriber.getSubscriberData() != null)
			{
				strResponse = (new CCWSCommandImpl()).getLogResponse(subscriber.getSubscriberData(), request.getIsdn());
			}
			else
			{
				strResponse = "ISDN=" + request.getIsdn() + " NULL";
			}
			instance.debugMonitor("RECEIVE:" + strResponse + ". ID=" + sessionId + ". costTime="
...
//			}
	...
	public SubscriberRetrieve getSubscriberInfo(CommandInstance instance, CommandMessage request) throws Exception
	{
		...
		try
		{
			...
			subscriber = connection.getSubscriberRetrieve(isdn, 1 + 8);
		}
		catch (Exception e)
		{
			throw e;
