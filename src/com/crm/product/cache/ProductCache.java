...
public class ProductCache
{
	...
	protected void loadProducts(Connection connection) throws Exception
	{
		...
		try
		{
			...
			while (rsConfig.next())
			{
				...
				product.setMerchantId(rsConfig.getLong("merchantId"));

				product.setOpId(rsConfig.getInt("telcoid"));

				product.setHost(rsConfig.getString("sync_host"));
	...
	protected synchronized void loadActions(Connection connection) throws Exception
	{
		...
		try
		{
			...
			String SQL = "Select * From ProductAction Order by productId desc, actionType desc, subscriberType desc";
			...
			while (rsConfig.next())
			{
				...
				action.setScenarioType(rsConfig.getLong("ScenarioID"));// DuyMB
																		// add
																		// scenario
																		// type
																		// for
	...
	protected synchronized void loadProductPrice(Connection connection) throws Exception
	{
		...
		try
		{
			...
			String SQL = "Select * From ProductPrice "
					+ "Order by productId desc, priceType desc, actionType desc, channel desc, segmentId desc, minQuantity desc";
	...
	protected synchronized void loadRoutes(Connection connection) throws Exception
	{
		...
		try
		{
			...
			while (rsConfig.next())
			{
				...
				route.setLanguageId(rsConfig.getString("languageId"));
