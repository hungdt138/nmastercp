...
public class OTPService
{
	...
	private static final int	GET_OTP_ERROR_404		= 404;
	private static final int	METHOD_NOT_ALLOWED_405	= 405;
	private static final int	NOT_ACCEPTABLE_406		= 406;
	private static final int	INVALID_ACTION_408		= 408;
	private static final int	NOT_SUCCESS_503			= 503;
			...
			final String product_code, final String action, final String username, final String salt) throws Exception
	{
		...
		try
		{
			...
			else if (response == GET_OTP_ERROR_404)
			{
				responseStatus = "OTP System Error";
			}
			...
			final String username, final String salt) throws Exception
	{
		...
		try
		{
			...
			else if (response == INVALID_ACTION_408)
			{
				responseStatus = "Only support actions: sub, unsubs";
			}
			else if (response == NOT_SUCCESS_503)
			{
				responseStatus = "Not success";
			}
		}
		catch (SocketTimeoutException se)
		{
	...
	// String moOutput = otps.createOTPRequest(OTPLINK, MSISDN, REQUEST_ID,
	// PRODUCT_CODE, ACTION, USERNAME, SALT);
	...
	// String subRequest = otps.createSUBRequest(SUBLINK, MSISDN, REQUEST_ID,
	// OTP, USERNAME, SALT);
