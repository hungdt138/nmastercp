 ...
 * @version 1.0
 */
public class CBConnection_bk extends ProvisioningConnection {
	...
	public CBConnection_bk() throws Exception {
		super();
	}
	...
	// DuyMB add check postpaid status 20131113
	public long getSubscriberStatus(String schema, String isdn) throws TreChainedException
	{
		String strCommand = 
			"select count(*) from " + schema + ".service_history_v a " +
			" where " +
			" a.service_type_id = 1000040 " +  // Postpaid type
			" and a.service_status_code in (3,211,2002) " + // ( active or bared 1 way)
			" and a.service_name = '" + isdn + "'" + //-- Service number
			" and sysdate between a.effective_start_date and a.effective_end_date";
		List listFieldNames = new ArrayList();
		List listFieldValues = new ArrayList();
		
		List listReturn = ltc.biSQLQuery(strCommand, listFieldNames, listFieldValues);
	...
	// DuyMB add end check postpaid status 20131113
	...
	public static int getSubscriberInfo(String isdn, CommandMessage request)
		...
		try {
			Hashtable mprtConfig = Global.loadProperties("ServerConfig.txt");
			Class.forName("oracle.jdbc.driver.OracleDriver");

			_connection = DriverManager.getConnection(
					StringUtil.nvl(mprtConfig.get("DBUrl"), ""),
			...
			cstmt = _connection.prepareCall(call);
	...
	public long getTimeUsed(String schema, String isdn, String registerDate)
			throws TreChainedException {
		try {
			schema = getSchema(schema);
			...
			String query = "select sum(ceil(nv.duration/60)) duration_in_minute "
					...
					+ "and nv.event_sub_type_code = 2601 "
					+ "and nv.c_party_id = '"
					+ isdn
					+ "' "
